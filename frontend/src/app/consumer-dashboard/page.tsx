...
type HistoryItem = { id?: string; vehicleId?: string; title: string; status: "OPEN" | "IN_PROGRESS" | "DONE" | "CANCELLED" | string; completedAt?: string; cost?: number; vehicle?: { plateNo?: string; make?: string; model?: string } };
...
export default function CustomerDashboard() {
    ...
    (async () => {
      try {
        const mePromise = api("/api/customer/customers/me").catch((e: any) => {
          ...
          if (msg.startsWith("404")) return null; // no profile yet
          throw e;
        });
        ...
        const sorted = hist.sort((a, b) => (b.completedAt || "").localeCompare(a.completedAt || ""));
        setRecent(sorted.slice(0, 8)); // show a few
      } catch (e: any) {
  ...
  // UI
  if (loading) return <div className="min-h-[60vh] grid place-items-center text-gray-600">Loading…</div>;
  if (err) return <div className="p-6"><div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-red-700">{err}</div></div>;

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Sidebar-ish top bar & hero */}
      <div
        className="border-b"
        style={{
        ...
        <div className="mx-auto max-w-7xl px-6 py-6 text-white">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-white/85 mt-1">Here’s a quick overview of your garage and recent services.</p>
            </div>
            <a
      ...
      <div className="mx-auto max-w-7xl px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard title="Active Jobs" value={String(stats.active)} hint="Open or in-progress" icon="🛠️" />
          <StatCard title="Completed" value={String(stats.completed)} hint="All-time shown" icon="✅" />
          <StatCard title="Recent Cost" value={formatLKR(stats.totalCost)} hint="Sum of recent items" icon="💳" />
        </div>
          ...
          <section className="lg:col-span-1">
            <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
              <div className="flex items-center justify-between">
                ...
                {me && <span className="rounded-full bg-white text-[#3E92CC] ring-1 ring-[#3E92CC]/30 px-2.5 py-1 text-xs font-medium">Customer</span>}
              </div>
              ...
              {me ? (
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <button
                    className="inline-flex items-center justify-center rounded-lg px-4 py-2 font-medium text-white shadow-sm"
        ...
        <section>
          <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
            ...
            <div className="mt-4">
              <a href="/customer-history" className="text-sm font-medium" style={{ color: PALETTE.sky }}>
                View all history →
              </a>
            </div>
          </div>
...
function formatLKR(n: number) {
  try { return n.toLocaleString("en-LK", { style: "currency", currency: "LKR", maximumFractionDigits: 0 }).replace("LKR", "Rs."); }
  catch { return `Rs. ${Math.round(n).toLocaleString()}`; }
}
